 ...
 */
export function createWsClient(auth: AuthManager, options?: WsClientOptions): WsClient {
	let _ws: WebSocket | null = null;
	let _sessionId: string | null = null;
	let _seq: number = 0;
	let _heartbeatInterval: number = DEFAULTS.WS_HEARTBEAT_MS;
	...
	/** 当前重连延迟（指数退避） */
	let _currentReconnectDelay: number = DEFAULTS.WS_RECONNECT_DELAY_MS;
	...
	/**
	 * 创建 connect() 用的 Promise 控制结构。
	 * resolve 延迟到 READY/RESUMED 事件后，确保鉴权真正成功。
	 */
	...
	let _connectControl: ConnectControl | null = null;
	...
	function rejectConnect(err: Error): void {
		if (_connectControl) {
			clearTimeout(_connectControl.timeout);
