...
export function createSessionManager() {
  ...
  function projectDirMatches(dirName: string, cwd: string): boolean {
   ...
   */
  function listSessions(cwd?: string): PiSessionInfo[] {
    try {
      ...
      for (const project of projects) {
        ...
        // 只显示当前目录对应的项目
        if (cwd && !projectDirMatches(project.name, cwd)) continue;
   ...
   */
  function parseSessionFile(raw: string, maxMessages: number): string {
   ...
   */
  function getSessionFilePreview(filePath: string, maxMessages: number = 10): string {
    try {
   ...
   */
  function getSessionDisplay(filePath: string): { customName?: string; lastUserMessage?: string } {
    try {
      const raw = readFileSync(filePath, "utf-8");
      ...
      for (let i = lines.length - 1; i >= 0; i--) {
        try {
          ...
          if (customName && lastUserMessage) break;
        } catch {
          // 跳过解析失败的行
        }
      }
   ...
   */
  function getSessionCwd(filePath: string): string | undefined {
    try {
      const raw = readFileSync(filePath, "utf-8");
   ...
   */
  function unencodeProjectDir(dirName: string): string {
  ...
  }): { text: string; total: number; totalPages: number; page: number } {
    const sessions = listSessions();
    const total = sessions.length;
    if (total === 0) return { text: "暂无 session", total: 0, totalPages: 0, page: 1 };
