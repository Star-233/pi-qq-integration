 ...
 */
export function createCommandHandler(api, sessionManager, callbacks) {
    /**
     * 尝试将文本作为命令处理。
     * @returns true 如果文本是命令且已处理，false 表示不是命令需按 prompt 处理
     */
    async function tryHandle(text, from) {
        if (!text.startsWith("#"))
            return false;
        ...
        switch (cmd) {
            ...
            case "clear":
                ...
                await api.sendMarkdown(from, `\`#${cmd}\` 已移除。用 \`#create <序号/名称>\` 创建复用指定 session 的新实例，\`#create new [--dir <目录>]\` 创建全新 session 的实例。`);
                return true;
            case "history":
    ...
    async function cmdHelp(session) {
    ...
    async function cmdClose(session, arg) {
        ...
        const lines = [];
        if (closed.length > 0)
            lines.push(`✅ 已关闭: ${closed.join(", ")}`);
        if (selfPid !== null) {
            lines.push(`🔄 正在关闭当前实例（${selfPid}），剩余实例将自动重新选举 leader ...`);
        }
        for (const f of failed)
            lines.push(`❌ ${f.pid}: ${esc(f.error)}`);
    ...
    async function cmdSettings(session, args) {
    ...
    async function cmdTarget(session) {
        ...
        await api.sendMarkdown(session, `✅ 已将当前会话设为默认转发目标：\`${session.type}\` \`${session.id}\``);
    }
    ...
    async function cmdInstances(session) {
    ...
    async function cmdTo(session, arg) {
        ...
        if (content) {
            ...
            await api.sendMarkdown(session, `✅ 已切换到实例 **${esc(displayName)}**，你的消息已送达：\`${esc(content.slice(0, 50))}\``, undefined, { claim: false });
        }
        else {
            // 确认回复不参与会话认领（claim 已由 reroute 切到目标实例）
            await api.sendMarkdown(session, `✅ 当前会话已切换到实例 **${esc(displayName)}**，之后的回复将路由到该实例。`, undefined, { claim: false });
        }
    }
    return { tryHandle };
}
