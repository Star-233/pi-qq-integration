...
export default function (pi: ExtensionAPI) {
	...
	function openSpawnLog(): number | "ignore" {
		try {
	...
	async function spawnInstance(opts: {
		...
		try {
			...
			info(
				`#create: 已 spawn 新实例 (sh=${child.pid}, cwd=${opts.cwd}${opts.sessionPath ? `, session=${opts.sessionPath}` : ""})`,
			);
			...
			while (Date.now() < deadline) {
				...
				const fresh = Object.values(reg.instances).find(
					(i) => i.role === "follower" && !before.has(i.id),
				);
				if (fresh) {
	...
	async function closeInstance(
		pid: number,
	): Promise<{ ok: boolean; self?: boolean; error?: string }> {
		...
		try {
			...
			return { ok: false, error: `发送 SIGTERM 失败: ${(err as Error).message}` };
		}
		// 等待退出，超时 SIGKILL
	...
	async function maybeTakeoverLeader(ctx: ExtensionContext): Promise<void> {
		if (_followerStop) return;
	...
	function claimSession(session: QBSession, lastMsg?: string): void {
		...
		else if (_role === "follower" && _ipcClient) _ipcClient.send({ type: "claim", sessionKey: key, info });
	}
	...
	pi.registerCommand("qq-status", {
		...
		handler: async (_args: string, ctx: ExtensionCommandContext) => {
			...
			if (_role === "follower") {
				...
				lines.push(
					`${ipcConnected ? "🟢" : "🔴"} **IPC**: ${ipcConnected ? "已连接 leader" : "未连接 leader（重连中）"}`,
				);
